...
Afterward, refresh the authenticated application session and verify `/account`, `/admin`, `/admin/audit`, `/mod`, and `/content`. Admin remains separate from subscriber entitlement: `/member` must remain denied unless professional age verification and an active subscription are independently present. A future production bootstrap procedure should add a trusted audit event and stronger two-person review.
