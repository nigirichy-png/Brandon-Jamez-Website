...
import type { StaffAccessDecision, StaffScenario } from "@/lib/staff/types";
import { InternalNavigation } from "./internal-navigation";
import { StaffScenarioSwitcher } from "./staff-scenario-switcher";
import { StatusLabel } from "./status-label";

export function InternalShell({ state, decision, currentPath, eyebrow, title, description, children }: { state: StaffScenario; decision: StaffAccessDecision; currentPath: string; eyebrow: string; title: string; description: string; children: React.ReactNode }) {
    ...
    <main id="main-content" className="flex-1 bg-[#090b10]">
      <section className="border-b border-white/10 bg-[#0d1016]">
        <div className="page-shell flex flex-wrap items-center justify-between gap-4 py-5">
          ...
          <div className="flex flex-wrap items-center gap-2"><StatusLabel tone="warning">Development preview</StatusLabel><Link href="/" className="inline-flex min-h-10 items-center rounded-lg px-3 text-sm font-bold text-zinc-400 hover:bg-white/[0.05] hover:text-white">Public website <span className="ml-1.5" aria-hidden="true">↗</span></Link></div>
        </div>
      </section>
      ...
      <div className="page-shell grid gap-6 py-8 lg:grid-cols-[16rem_minmax(0,1fr)] lg:gap-8 lg:py-10">
        <aside className="self-start rounded-2xl border border-white/10 bg-[#10131a] p-4 lg:sticky lg:top-24">
          <p className="text-xs font-extrabold uppercase tracking-[0.13em] text-zinc-600">Simulated operator</p>
        ...
        <div className="min-w-0">
          <header className="border-b border-white/10 pb-7">
            <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-xs font-extrabold uppercase tracking-[0.15em] text-fuchsia-300">{eyebrow}</p><StatusLabel tone={decision.allowed ? "positive" : "danger"}>{decision.allowed ? "Preview authorized" : "Preview denied"}</StatusLabel></div>
          ...
          <div className="my-6"><StaffScenarioSwitcher activeScenario={state.scenarioId} currentPath={currentPath} /></div>
          {children}
        </div>
      </div>
    </main>
