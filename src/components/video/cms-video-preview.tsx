...
export function CmsVideoPreview({ title, platform, videoUrl, priority = false, compact = false, sizes }: CmsVideoPreviewProps) {
  const thumbnailUrl = getVideoThumbnailUrl(platform, videoUrl);
  const identity = videoPlatformIdentities[platform];

  return <div className={`relative aspect-video overflow-hidden bg-gradient-to-br ${identity.previewClass}`}>
