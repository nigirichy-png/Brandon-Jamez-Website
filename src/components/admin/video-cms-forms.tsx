...
export function CmsVideoRecord({ video }: { video: CmsVideo }) {
  ...
  return <article className={`rounded-2xl border bg-[#12151c] p-5 sm:p-6 ${video.featured ? "border-fuchsia-300/35 shadow-[0_18px_50px_rgba(229,79,236,0.08)]" : "border-white/10"}`}>
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><div className="flex flex-wrap items-center gap-2"><StatusLabel tone={video.status === "published" ? "positive" : "warning"}>{video.status}</StatusLabel><StatusLabel tone="info">{cmsPlatformLabels[video.platform]}</StatusLabel>{video.featured ? <StatusLabel tone="positive">Featured</StatusLabel> : null}</div><h2 className="font-display mt-4 text-2xl font-bold text-white">{video.title}</h2><p className="mt-2 max-w-2xl leading-6 text-zinc-400">{video.short_description || "No short description."}</p></div>
      <a href={video.video_url} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center rounded-xl border border-white/15 px-4 text-sm font-extrabold text-cyan-100 hover:border-cyan-300/40">Open {cmsPlatformLabels[video.platform]} <span className="ml-2" aria-hidden="true">↗</span></a>
    </div>
    <dl className="mt-5 grid gap-3 border-y border-white/10 py-4 text-sm sm:grid-cols-2 xl:grid-cols-4">
      <div><dt className="text-zinc-500">Category</dt><dd className="mt-1 font-bold text-zinc-200">{video.category ?? "Uncategorized"}</dd></div>
