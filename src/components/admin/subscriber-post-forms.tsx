...
function Fields({ post }: { post?: AdminSubscriberPost }) {
  return <div className="grid gap-3.5">
    ...
    <div className="grid gap-3.5 md:grid-cols-2">
      <label className="text-sm font-bold text-zinc-200">External cover image URL <span className="font-normal text-zinc-500">(not private fallback)</span><input className={field} name="coverImageUrl" defaultValue={post?.cover_image_url ?? ""} type="url" /></label>
    ...
    <div className="rounded-lg border border-amber-300/20 bg-amber-300/[0.04] p-3.5">
      <p className="text-sm font-extrabold text-amber-100">External media is not private</p>
      <p className="mt-1 text-xs leading-5 text-zinc-400">The subscriber page protects the post, but it cannot prevent an external provider or anyone who knows the external URL from opening that media elsewhere. Use the private MP4/WebM upload below for protected files.</p>
      <div className="mt-3 grid gap-3.5 md:grid-cols-2">
        <label className="text-sm font-bold text-zinc-200">External media URL<input className={field} name="mediaUrl" defaultValue={post?.media_url ?? ""} type="url" /><span className="mt-2 block text-xs font-normal leading-5 text-zinc-500">Direct video must use HTTPS. Embeds accept only normalized YouTube or Vimeo URLs.</span></label>
        <label className="text-sm font-bold text-zinc-200">External media type<select className={field} name="mediaType" defaultValue={post?.media_type ?? ""}><option value="">None</option><option value="image">External image</option><option value="video">External direct HTTPS video</option><option value="embed">External YouTube or Vimeo embed</option></select></label>
      </div>
    </div>
  </div>;
}
...
export function CreateSubscriberPostForm() {
  ...
  return <form action={action} className="rounded-xl border border-white/10 bg-[#12151c] p-4 sm:p-5"><h2 className="font-display text-xl font-bold text-white">Create subscriber post</h2><p className="mb-4 mt-1.5 text-sm text-zinc-400">Create the post first, then upload private images or a small private video from its record. External URLs remain clearly labeled compatibility fallbacks.</p><Fields /><button disabled={pending} className="mt-4 min-h-10 rounded-lg bg-white px-4 text-sm font-extrabold text-black hover:bg-zinc-200 disabled:opacity-40">{pending ? "Saving…" : "Create post"}</button><Message state={state} /></form>;
}
...
type PostWithMedia = AdminSubscriberPost & { cover_image_src: string | null; content_image_src: string | null; private_video_src: string | null };

function ImageManager({ post, kind, imageSrc, hasPrivateImage }: { post: PostWithMedia; kind: "cover" | "content"; imageSrc: string | null; hasPrivateImage: boolean }) {
  ...
  return <section className="rounded-lg border border-white/10 bg-black/15 p-3.5"><h3 className="font-display text-base font-bold text-white">{label}</h3>{imageSrc ? <img src={imageSrc} alt="" className="mt-2.5 aspect-video max-h-56 w-full rounded-lg border border-white/10 object-contain" /> : <p className="mt-2 text-sm text-zinc-500">No image configured.</p>}<form action={uploadAction} className="mt-3"><label className="text-sm font-bold text-zinc-200">{hasPrivateImage ? "Replace private image" : "Upload private image"}<input className={`${field} file:mr-3 file:rounded-md file:border-0 file:bg-zinc-200 file:px-3 file:py-1.5 file:font-bold file:text-black`} name="image" type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif" required /></label><p className="mt-1.5 text-xs leading-5 text-zinc-500">JPEG, PNG, WebP, GIF, or AVIF. Maximum 10 MB.</p><button disabled={uploading} className="mt-2.5 min-h-10 rounded-lg border border-cyan-300/30 px-3.5 text-sm font-extrabold text-cyan-100">{uploading ? "Uploading…" : hasPrivateImage ? "Replace image" : "Upload image"}</button><Message state={uploadState} /></form>{hasPrivateImage ? <form action={removeAction} className="mt-2.5" onSubmit={(event) => { if (!window.confirm(`Remove the private ${kind} image from “${post.title}”?`)) event.preventDefault(); }}><button disabled={removing} className="min-h-10 rounded-lg border border-rose-300/30 px-3.5 text-sm font-extrabold text-rose-100">{removing ? "Removing…" : "Remove private image"}</button><Message state={removeState} /></form> : null}</section>;
}

function VideoManager({ post }: { post: PostWithMedia }) {
  const upload = uploadSubscriberPostVideoAction.bind(null, post.id, post.updated_at, post.slug);
  const remove = removeSubscriberPostVideoAction.bind(null, post.id, post.updated_at, post.slug);
  const [uploadState, uploadAction, uploading] = useActionState(upload, initial);
  const [removeState, removeAction, removing] = useActionState(remove, initial);
  return <section className="rounded-lg border border-cyan-300/20 bg-cyan-300/[0.025] p-3.5"><h3 className="font-display text-base font-bold text-white">Private Storage video</h3>{post.private_video_src ? <video src={post.private_video_src} controls preload="metadata" className="mt-2.5 aspect-video max-h-56 w-full rounded-lg border border-white/10 bg-black" /> : <p className="mt-2 text-sm text-zinc-500">No private video configured.</p>}<form action={uploadAction} className="mt-3"><label className="text-sm font-bold text-zinc-200">{post.video_path ? "Replace private video" : "Upload private video"}<input className={`${field} file:mr-3 file:rounded-md file:border-0 file:bg-zinc-200 file:px-3 file:py-1.5 file:font-bold file:text-black`} name="video" type="file" accept="video/mp4,video/webm" required /></label><p className="mt-1.5 text-xs leading-5 text-zinc-500">MP4 or WebM, maximum 10 MB. Delivered through a fresh entitlement check and a short-lived server-side Storage signature. Intended for small clips, not large-scale streaming.</p><button disabled={uploading} className="mt-2.5 min-h-10 rounded-lg border border-cyan-300/30 px-3.5 text-sm font-extrabold text-cyan-100">{uploading ? "Uploading…" : post.video_path ? "Replace video" : "Upload video"}</button><Message state={uploadState} /></form>{post.video_path ? <form action={removeAction} className="mt-2.5" onSubmit={(event) => { if (!window.confirm(`Remove the private video from “${post.title}”?`)) event.preventDefault(); }}><button disabled={removing} className="min-h-10 rounded-lg border border-rose-300/30 px-3.5 text-sm font-extrabold text-rose-100">{removing ? "Removing…" : "Remove private video"}</button><Message state={removeState} /></form> : null}</section>;
}

export function SubscriberPostRecord({ post }: { post: PostWithMedia }) {
