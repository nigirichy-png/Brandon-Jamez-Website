...
export default async function SubscriberPage() {
  ...
  return <main id="main-content" className="platform-page platform-subscriber-page flex-1">
    <header className="platform-page-header platform-subscriber-header"><div className="platform-shell platform-subscriber-header-grid"><div><div className="platform-member-label"><span>Subscriber area</span><b>18+</b></div><h1 className="platform-title">Your exclusive <span>Pattaya feed.</span></h1><p className="platform-copy">Welcome, {state.displayName ?? "Subscriber"}. This is your subscriber-only space for exclusive videos, member posts and private Pattaya content.</p><div className="platform-member-tags"><span>Subscriber-only videos</span><span>Exclusive member posts</span><span>Private member access</span></div></div><aside className="platform-member-status"><small>Access status</small><strong>Subscriber access active</strong><p>Your access is active. Enjoy all currently published subscriber content.</p><Link href="/account" className="platform-button-secondary">Manage account</Link></aside></div></header>
    {videos.length ? <section className="platform-shell platform-subscriber-library" aria-labelledby="subscriber-videos-title"><div className="platform-list-heading"><div><p className="platform-kicker">Subscriber content</p><h2 id="subscriber-videos-title">Member videos</h2></div><span>{videos.length} published</span></div><div className="platform-video-grid">{videos.map((video) => <SubscriberVideoCard key={video.id} video={video} />)}</div></section> : null}
