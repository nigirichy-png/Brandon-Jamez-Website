...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string | string[]; status?: string | string[]; next?: string | string[] }> }) {
  ...
  return <AuthShell eyebrow="Brandon Jamez account" title="Welcome back." description="Sign in through the server-rendered Supabase authentication flow. Sessions are stored through the existing SSR cookie boundary.">
    ...
    <div className="mt-7 rounded-[var(--radius-md)] border border-amber-300/20 bg-amber-300/[0.05] p-4"><p className="text-sm font-bold text-amber-100">Development previews</p><p className="mt-1 text-xs leading-5 text-amber-100/60">These links explicitly select isolated mock scenarios. They do not upgrade a real session.</p><div className="mt-3 grid gap-2 min-[430px]:grid-cols-2"><Link href="/member?demo=active_subscriber" className="flex min-h-11 items-center rounded-lg px-2 text-sm font-extrabold text-cyan-300 hover:bg-white/[0.04]">Subscriber demo <span className="ml-auto" aria-hidden="true">→</span></Link><Link href="/mod?staffDemo=moderator" className="flex min-h-11 items-center rounded-lg px-2 text-sm font-extrabold text-cyan-300 hover:bg-white/[0.04]">Moderator demo <span className="ml-auto" aria-hidden="true">→</span></Link><Link href="/content?staffDemo=content_manager" className="flex min-h-11 items-center rounded-lg px-2 text-sm font-extrabold text-cyan-300 hover:bg-white/[0.04]">Content demo <span className="ml-auto" aria-hidden="true">→</span></Link><Link href="/admin?staffDemo=admin" className="flex min-h-11 items-center rounded-lg px-2 text-sm font-extrabold text-cyan-300 hover:bg-white/[0.04]">Admin demo <span className="ml-auto" aria-hidden="true">→</span></Link></div></div>
  </AuthShell>;
}
