...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<{ staffDemo?: string | string[]; page?: string | string[]; error?: string | string[] }> }) {
  ...
  return <InternalShell state={state} decision={decision} currentPath="/admin/users" eyebrow="Administration · users" title="Minimum data, explicit privilege." description={state.developmentPreview ? "Fictional account summaries remain available only in the local development preview." : "Review data-minimized real account state. Every privileged mutation repeats authorization and is audited on the server."}>
