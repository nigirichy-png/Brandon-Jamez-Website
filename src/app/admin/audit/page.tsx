...
export default async function AdminAuditPage({ searchParams }: { searchParams: Promise<{ staffDemo?: string | string[]; page?: string | string[] }> }) {
  ...
  return <InternalShell state={state} decision={decision} currentPath="/admin/audit" eyebrow="Administration · audit" title="Audit activity" description={state.developmentPreview ? "Fictional audit records for local development." : "Review administrative activity recorded by trusted database operations."}>
