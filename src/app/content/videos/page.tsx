...
export default async function ContentVideosPage({ searchParams }: { searchParams: Promise<{ staffDemo?: string | string[] }> }) {
  ...
  const videos = decision.allowed && !state.developmentPreview ? await listStaffCmsVideos().catch(() => null) : [];
  const canEdit = state.roles.includes("content_manager") || state.roles.includes("admin");
  return <InternalShell state={state} decision={decision} currentPath="/content/videos" eyebrow="Content · video records" title="Video records" description={canEdit ? "Manage supported external video metadata through audited content actions." : "Review supported external video metadata with read-only moderator access."}>
    {!decision.allowed ? <StaffAccessGate decision={decision} area="content" /> : state.developmentPreview ? <section className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.05] p-6"><StatusLabel tone="warning">Real session required</StatusLabel><h2 className="font-display mt-4 text-2xl font-bold text-white">CMS data is unavailable in preview mode.</h2></section> : videos === null ? <p role="alert" className="rounded-2xl border border-rose-300/20 bg-rose-300/[0.06] p-6 text-rose-100">Video records could not be loaded safely.</p> : <div className="space-y-8">{canEdit ? <CreateCmsVideoForm /> : null}<section aria-labelledby="video-records-title"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><h2 id="video-records-title" className="font-display text-2xl font-bold text-white">Video inventory</h2><StatusLabel tone={canEdit ? "positive" : "neutral"}>{canEdit ? "Audited editor" : "Read only"}</StatusLabel></div>{videos.length ? <div className="grid gap-5">{videos.map((video) => <CmsVideoRecord key={video.id} video={video} readOnly={!canEdit} />)}</div> : <p className="rounded-2xl border border-dashed border-white/15 p-6 text-center text-sm text-zinc-400">No video records yet.</p>}</section></div>}
  </InternalShell>;
}
