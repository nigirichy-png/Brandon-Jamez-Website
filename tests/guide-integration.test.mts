...
test("guide route is native, read-only, app-like on mobile, and builder-compatible", async () => {
  ...
  assert.match(map,/^"use client"/); assert.doesNotMatch(map,/guide-mobile-map/); assert.match(map,/import\("\.\/guide-mapbox-map"\)/); assert.match(mapboxMap,/ResizeObserver/); assert.match(mapboxMap,/streets-v12/); assert.match(mapboxMap,/cluster: true/); assert.match(mapboxMap,/guide-cluster-count/); assert.match(mapboxMap,/guide-point-icons/); assert.doesNotMatch(guide,/next\/dynamic/); assert.match(guide,/data-site-builder-kind="dynamic"/); assert.doesNotMatch(guide,/dangerouslySetInnerHTML/);
